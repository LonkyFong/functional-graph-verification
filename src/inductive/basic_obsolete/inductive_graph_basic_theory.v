...
Require Import GraphVerification.src.inductive.basic.inductive_graph_basic_to_RG.
...
Lemma _In_labNodes_is_InMap : forall (x : Node) (ig : IG_basic),
...
Lemma _updateEntry_does_not_change_key_set : forall (node : Node) (f : NatSet.t * NatSet.t -> NatSet.t * NatSet.t) (ig : IG_basic) (x : Node),
...
Lemma _insEdge_does_not_add_node : forall (edge : (Node * Node)) (ig : IG_basic) (x : Node),
...
Lemma _insEdges_does_not_add_nodes : forall (edges : list (Node * Node)) (ig : IG_basic) (x : Node),
...
Lemma _insNode_any_ins_node : forall (node : Node) (ig : IG_basic) (x : Node),
...
Lemma _insNodes_any_ins_all_nodes : forall (nodes : list Node) (ig : IG_basic) (x : Node),
...
(* "big" statement: *)
Theorem IG_basic_mkGraph_any_ins_all_nodes : forall (nl : list Node) (el : list (Node * Node)) (x : Node),
  In x (IG_basic_labNodes (IG_basic_mkGraph nl el)) <-> In x nl.
Proof.
  intros. unfold IG_basic_mkGraph. rewrite _insEdges_does_not_add_nodes. rewrite _insNodes_any_ins_all_nodes.
  rewrite app_nil_r. reflexivity.
Qed.


Theorem IG_basic_empty_isEmpty : IG_basic_isEmpty IG_basic_empty = true.
Proof.
  compute. reflexivity.
Qed.

Theorem IG_basic_labNodes_empty_nil : IG_basic_labNodes IG_basic_empty = [].
Proof.
  compute. reflexivity.
Qed.
...
Proof.
  intros. unfold IG_basic_isEmpty. rewrite <- _not_NatMap_Empty_is_empty_false. unfold not.
  ...
  - firstorder.
    + apply MProps.elements_Empty in H0.
      ...
      apply IG_basic_mkGraph_any_ins_all_nodes. simpl. left. reflexivity.
    + congruence.
Qed.



Theorem IG_basic_matsh_empty_is_nothing : forall (node : Node), IG_basic_match node IG_basic_empty = (None, IG_basic_empty).
Proof.
  intros. compute. reflexivity.
Qed.


Theorem IG_basic_spec4 : forall (node : Node) (nodes : list Node) (edges : list (Node * Node)), 
...
Theorem IG_basic_spec5 : forall (node : Node) (nodes : list Node) (edges : list (Node * Node)), 
